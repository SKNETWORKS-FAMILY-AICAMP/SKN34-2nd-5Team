...
- 비로그인 상태 상단: `회원가입 / 로그인`
- 로그인 상태 상단: `로그아웃`
- 관리자 로고 클릭: `/auth/admin`
- 일반 회원 로고 클릭: `/auth/profile`
- 비로그인 사용자 로고 클릭: `/auth/login`
- 로그인 사용자가 `/auth/login` 또는 `/auth/signup`에 직접 접근하면 자신의 화면으로 이동
- 관리자 로그인 성공: `/auth/admin`
- 일반 회원 로그인 성공: `AUTH_AFTER_LOGIN_URL`, 기본값 `/auth/profile`

React 연결 후 일반 회원을 운영 화면으로 보낼 경우 서버 환경변수를 다음과 같이 설정할 수 있다.

```dotenv
AUTH_AFTER_LOGIN_URL=/
```

관리자는 현재 승인 화면으로 이동한다. 관리 후 React로 이동시키려면 `/auth/admin`에 `운영 서비스로 이동` 링크를 추가하거나, React 상단에서 `/auth/admin`으로 들어갈 수 있는 관리자 메뉴를 제공한다.
