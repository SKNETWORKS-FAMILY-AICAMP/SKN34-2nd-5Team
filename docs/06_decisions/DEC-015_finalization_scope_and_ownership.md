...
| C03 | OOF / Holdout / Test 용어 통일 | 필수 | 문서 보완 필요 | 모델 담당 / 이홍규 문서 통합 | README와 최종 문서에서 정의 일치 | `docs/final-product-story` |
| C04 | `risk_score ≠ 이탈확률` UI·문서 명시 | 필수 | 완료 | 이홍규 | 현재 경고 문구와 표현 원칙 유지 | 완료 근거 유지 |
| C05 | F1 외 Top-K·Lift 운영 지표 강조 | 필수 | 문서 보완 필요 | 모델 담당 확인 / 이홍규 문서 통합 | README에서 모델 성능과 운영 용량 연결 | `docs/final-product-story` |
| C06 | 모델·코호트·DB 버전 역할 구분 | 필수 | 문서 보완 필요 | 모델 담당 / 이홍규 문서 통합 | `코호트 v04`, `운영 모델 v05_05_dl`, DB `model_version` 역할을 구분하며 억지로 동일화하지 않음 | `docs/final-product-story` |
...
C 영역의 기술적 판정은 모델 담당자가 소유하며, 이홍규는 모델 담당자가 확인한 내용을 최종 제품·README 문서에 통합한다.
