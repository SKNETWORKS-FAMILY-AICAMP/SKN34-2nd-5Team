"""Load reviewed v05 city operating artifacts into yelp_data.

The loader applies only v05/database/ddl/020_create_city_operating_tables.sql,
refuses to append when v04 rows already exist, and loads both tables in one
transaction.
"""
...
DDL_PATH = (
    ...
    / "database"
    / "ddl"
    / "020_create_city_operating_tables.sql"
)
...
ARTIFACTS = (
    Artifact(
        "city_review_supply",
        "city_review_supply_v05.parquet",
        (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Load reviewed v05 city operating Parquet artifacts."
    )
    parser.add_argument(
        "--confirm-database",
        required=True,
...
def load_frames() -> dict[str, pd.DataFrame]:
    ...
    for artifact in ARTIFACTS:
        ...
        for flag in ("minimum_sample_met", "is_comparison_year", "is_selection_year"):
            if flag in frame:
