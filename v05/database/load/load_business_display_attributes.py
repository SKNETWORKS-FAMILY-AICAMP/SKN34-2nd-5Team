"""Load v05 display-only business attributes into the existing yelp_data DB."""
from __future__ import annotations

import argparse
from pathlib import Path
import sys

import pandas as pd


PROJECT_ROOT = Path(__file__).resolve().parents[3]
DATABASE_LOAD_DIR = PROJECT_ROOT / "database" / "load"
if str(DATABASE_LOAD_DIR) not in sys.path:
    sys.path.insert(0, str(DATABASE_LOAD_DIR))

from load_v04 import create_engine_from_env, database_name, sql_statements  # noqa: E402


TABLE = "business_display_attribute"
ARTIFACT_PATH = (
    PROJECT_ROOT / "data" / "processed" / "business_display_attributes_v01.parquet"
)
DDL_PATH = (
    PROJECT_ROOT / "v05" / "database" / "ddl" / "016_create_business_display_attributes.sql"
)
COLUMNS = (
    "business_id",
    "address",
    "postal_code",
    "is_open_snapshot",
    "hours_json",
    "price_range",
    "takeout",
    "delivery",
    "reservations",
    "outdoor_seating",
    "wifi",
    "parking_json",
    "wheelchair_accessible",
    "alcohol",
    "source_type",
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Load display-only Yelp business attributes into yelp_data."
    )
    parser.add_argument(
        "--confirm-database",
        required=True,
        help="Safety check: enter the exact target database name.",
    )
    parser.add_argument("--chunk-size", type=int, default=1000)
    return parser.parse_args()


def load_frame() -> pd.DataFrame:
    if not ARTIFACT_PATH.is_file():
        raise FileNotFoundError(ARTIFACT_PATH)
    frame = pd.read_parquet(ARTIFACT_PATH)
    missing = sorted(set(COLUMNS) - set(frame.columns))
    if missing:
        raise ValueError(f"missing columns: {missing}")
    frame = frame.loc[:, COLUMNS].copy()
    if frame["business_id"].isna().any():
        raise ValueError("NULL business_id")
    if frame["business_id"].duplicated().any():
        raise ValueError("duplicate business_id")
    if set(frame["source_type"].dropna().astype(str)) != {"yelp_open_dataset"}:
        raise ValueError("unexpected source_type")
    if not frame["price_range"].dropna().between(1, 4).all():
        raise ValueError("price_range outside 1..4")
    return frame.where(pd.notna(frame), None)


def apply_schema(connection) -> None:
    if not DDL_PATH.is_file():
        raise FileNotFoundError(DDL_PATH)
    for statement in sql_statements(DDL_PATH):
        connection.exec_driver_sql(statement)
    print(f"schema applied: {DDL_PATH.name}")


def main() -> int:
    args = parse_args()
    frame = load_frame()
    engine = create_engine_from_env(PROJECT_ROOT)
    try:
        with engine.begin() as connection:
            actual_database = database_name(connection)
            if actual_database != args.confirm_database:
                raise RuntimeError(
                    f"connected DB {actual_database!r} does not match "
                    f"confirmation {args.confirm_database!r}"
                )
            apply_schema(connection)
            existing = int(
                connection.exec_driver_sql(f"SELECT COUNT(*) FROM {TABLE}").scalar_one()
            )
            if existing:
                raise RuntimeError(
                    f"{TABLE} already has {existing:,} rows; no data was appended"
                )
            frame.to_sql(
                TABLE,
                con=connection,
                if_exists="append",
                index=False,
                chunksize=args.chunk_size,
                method="multi",
            )
            loaded = int(
                connection.exec_driver_sql(f"SELECT COUNT(*) FROM {TABLE}").scalar_one()
            )
            if loaded != len(frame):
                raise RuntimeError(f"loaded {loaded:,}, expected {len(frame):,}")
            print(f"loaded: {TABLE} ({loaded:,} rows)")
        return 0
    finally:
        engine.dispose()


if __name__ == "__main__":
    raise SystemExit(main())
