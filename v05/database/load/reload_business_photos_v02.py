"""Replace only recommendation-scoped business photo rows."""
...
def main() -> int:
    ...
    if frame["photo_id"].isna().any() or frame["photo_id"].duplicated().any():
        raise ValueError("invalid photo_id key")
    ...
    try:
        with engine.begin() as connection:
            ...
            existing = int(connection.exec_driver_sql(f"SELECT COUNT(*) FROM {TABLE}").scalar_one())
            ...
            loaded = int(connection.exec_driver_sql(f"SELECT COUNT(*) FROM {TABLE}").scalar_one())
            if loaded != len(frame):
                raise RuntimeError(f"loaded {loaded:,}, expected {len(frame):,}")
            print(f"replaced {existing:,} rows with {loaded:,} business photo rows")
        return 0
    finally:
        engine.dispose()
